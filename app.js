...
let db = new sequelize('playlist_app', process.env.POSTGRES_USER, process.env.POSTGRES_PASSWORD, {
	server:'localhost',
	dialect: 'postgres'
})
...
db.sync({force: false}).then(db => {
	console.log('We synced guys!')
})

app.listen(3001,()=>{
	console.log("Your playlist_app is listens to port 3001!")
})